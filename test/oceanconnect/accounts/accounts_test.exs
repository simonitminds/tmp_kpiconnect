defmodule Oceanconnect.AccountsTest do
  ...
  describe "users" do
    ...
    test "list_users/0 returns all users", %{user: user, inactive_user: inactive_user} do
		...
		test "list_users/1 returns a paginated list of users", %{user: user, inactive_user: inactive_user} do
			page = Accounts.list_users(%{})
			assert page.entries == [user, inactive_user]
		end

		test "list_active_users/0 returns all users marked as active", %{user: user, inactive_user: inactive_user} do
			assert Enum.map(Accounts.list_active_users(), fn(f) -> f.id end) == [user.id]
			refute Enum.map(Accounts.list_active_users(), fn(f) -> f.id end) == [user.id, inactive_user.id]
		end
    ...
    test "get_user!/1 returns the user with given id", %{user: user} do
      assert Accounts.get_user!(user.id) == user
		...
		test "get_active_user!/1 returns the active user with given id", %{user: user, inactive_user: inactive_user} do
  ...
  describe "companies" do
    ...
    @valid_attrs %{address1: "some address", contact_name: "some contact_name", country: "some country"}
    ...
    setup do
      ...
      {:ok, %{company: Accounts.get_company!(company.id), inactive_company: Accounts.get_company!(inactive_company.id)}}
    end

    test "list_companies/0 returns all companies", %{company: company, inactive_company: inactive_company} do
      assert Enum.map(Accounts.list_companies(), fn(f) -> f.id end) == [company.id, inactive_company.id]
    end
		...
		test "list_active_companies/0 returns all companys marked as active", %{company: company, inactive_company: inactive_company} do
			assert Enum.map(Accounts.list_active_companies(), fn(f) -> f.id end) == [company.id]
			refute Enum.map(Accounts.list_active_companies(), fn(f) -> f.id end) == [company.id, inactive_company.id]
		end
    ...
    test "get_company!/1 returns the company with given id", %{company: company, inactive_company: inactive_company} do
      ...
      assert Accounts.get_company!(inactive_company.id) == inactive_company
		...
		test "get_active_company!/1 returns the active company with given id", %{company: company, inactive_company: inactive_company} do
			assert Accounts.get_active_company!(company.id) == company
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_active_company!(inactive_company.id) end
		end

    test "create_company/1 with valid data creates a company" do
      assert {:ok, %Company{} = company} = Accounts.create_company(Map.merge(@valid_attrs, %{name: "some name"}))
