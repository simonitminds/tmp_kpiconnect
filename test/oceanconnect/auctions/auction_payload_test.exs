defmodule Oceanconnect.Auctions.AuctionPayloadTest do
  ...
  describe "get_auction_payload!/1" do
    setup do
      ...
      {:ok,
       %{auction: auction, supplier: supplier, supplier_2: supplier_2, supplier_user: supplier_user, supplier_user2: supplier_user2, fuel_id: fuel_id}}
    end
    ...
    test "anonymous_bidding", %{
      ...
      IO.inspect(Repo.all(Oceanconnect.Auctions.AuctionSuppliers))
      IO.inspect([supplier.id, auction.id], label: "Supplier ID and Auction ID")
      IO.inspect(Auctions.get_auction_supplier(auction.id, supplier.id), label: "SUPPLIER")
