defmodule Oceanconnect.Auctions.AuctionPayloadTest do
  ...
  describe "get_auction_payload!/1" do
    ...
    test "anonymous_bidding", %{auction: auction, supplier: supplier, bid_params: bid_params = %{"amount" => amount}, supplier_2: supplier_2}do
      ...
      assert Auctions.get_auction_supplier(auction.id, supplier.id).alias_name in Enum.map(buyer_payload.bid_history, &(&1.supplier))
      assert Auctions.get_auction_supplier(auction.id, supplier_2.id).alias_name in Enum.map(buyer_payload.bid_history, &(&1.supplier))
      assert Auctions.get_auction_supplier(auction.id, supplier_2.id).alias_name in Enum.map(buyer_payload.lowest_bids, &(&1.supplier))
    end
    ...
    test "includes submitted barges from supplier for buyer", %{auction: auction, supplier: supplier, supplier_2: supplier2} do
      barge = insert(:barge, companies: [supplier])
