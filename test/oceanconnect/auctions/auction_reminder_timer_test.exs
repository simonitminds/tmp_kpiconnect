defmodule Oceanconnect.Auctions.AuctionReminderTimerTest do
  ...
  setup do
    {:ok, test_start_time} = DateTime.to_unix(DateTime.utc_now(), :millisecond) + 60_200
    ...
    Enum.each(supplier_companies, fn(supplier_company) -> insert(:user, company: supplier_company) end)

    auction = insert(:auction, scheduled_start: test_start_time, suppliers: supplier_companies, buyer: buyer_company)
  ...
  test "start auction_reminder_timer for auction", %{auction: auction} do
    :timer.sleep(500)
    assert length(Enum.filter(AuctionEventStore.event_list(auction.id), fn(event) -> event.type == :upcoming_auction_notified end)) == 1
  end
end
