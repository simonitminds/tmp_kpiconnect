defmodule Oceanconnect.Notifications.EmailNotificationStoreTest do
  ...
  alias Oceanconnect.Auctions.{AuctionEvent, EventNotifier, AuctionStore.AuctionState, Solution, AuctionsSupervisor}
  ...
  describe "email notifications store" do
    setup do
      ...
      auction_state = %AuctionState{product_bids: product_bids} = Auctions.get_auction_state!(auction)
      ...
      {:ok, %{auction: auction, vessel_name_list: vessel_name_list, port_name: port_name, buyer: buyer, completed_auction_state: completed_auction_state}}
    end
    ...
    test "auction cancellation event produces enail", %{auction: auction, vessel_name_list: vessel_name_list, port_name: port_name, buyer: buyer} do
      auction_state = AuctionState.from_auction(auction)
    ...
    test "auction completion event produces email", %{auction: auction, completed_auction_state: completed_auction_state, buyer: buyer} do
