defmodule OceanconnectWeb.Api.PortSupplierControllerTest do
  ...
  describe "suppliers for port" do
    test "lists all suppliers for a port", %{conn: conn, buyer_company: buyer_company, port_suppliers: port_suppliers, port: port} do
    ...
    test "does not include suppliers from other ports", %{conn: conn, buyer_company: buyer_company, different_port_supplier: different_port_supplier, port: port} do
      conn = get conn, port_supplier_path(conn, :index, port, %{"buyer_id" => buyer_company.id})

      refute different_port_supplier in conn.assigns[:suppliers]
    end

    test "does not include the buyer's company", %{conn: conn, buyer_company: buyer_company, port: port} do
      conn = get conn, port_supplier_path(conn, :index, port, %{"buyer_id" => buyer_company.id})
