defmodule OceanconnectWeb.Api.PortSupplierControllerTest do
  ...
  setup do
   ...
   {:ok, conn: authed_conn, port_suppliers: supplier_companies, port: port, buyer_company: buyer_company, different_port_supplier: different_port_supplier}
  end


  describe "suppliers for port" do
    test "lists all suppliers for a port", %{conn: conn, port_suppliers: port_suppliers, port: port} do
      conn = get conn, port_supplier_path(conn, :index, port)

      assert json_response(conn, 200)
      assert conn.assigns[:suppliers] == port_suppliers
    end

    test "does not include suppliers from other ports", %{conn: conn, different_port_supplier: different_port_supplier, port: port} do
      conn = get conn, port_supplier_path(conn, :index, port)

      refute different_port_supplier in conn.assigns[:suppliers]
    end

    test "does not include the buyers company", %{conn: conn, port: port, buyer_company: buyer_company} do
      conn = get conn, port_supplier_path(conn, :index, port)
