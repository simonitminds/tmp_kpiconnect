defmodule Oceanconnect.AuctionShowPage do
  ...
  def delete_comment(index) when is_integer(index) do
    execute_script(
      "document.getElementsByClassName('qa-auction-solution-comment-delete')[#{index}].click()"
    )
  end
  ...
  def update_claim(claim_id) do
    click({:css, ".qa-auction-claims-update_claim-#{claim_id}"})
  end

  def view_claim(claim_id) do
    click({:css, ".qa-auction-claims-view_claim-#{claim_id}"})
  end
