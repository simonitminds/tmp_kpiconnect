defmodule OceanconnectWeb.Api.ObserverController do
  ...
  def invite(conn, %{"auction_id" => auction_id, "user_id" => user_id}) do
    ...
    with %{is_admin: true} = current_user <- Auth.current_user(conn),
         %struct{} = auction when is_auction(struct) <- Auctions.get_auction(auction_id),
         available_observers <- Accounts.list_observers(),
         %User{is_observer: true} = user <- Enum.find(available_observers, & &1.id == String.to_integer(user_id)) do
      ...
      conn
      |> render("invite.json", aution_payload: auction_payload)
    else
      _ ->
        conn
        |> put_status(422)
