...
>>>>>>> Event Sourcing 2: Non-eclectic Boogaloo
  ...
  def apply(
        ...
        %AuctionEvent{type: :auction_updated, data: %Auction{
  ...
  def apply(
        state = %AuctionState{auction_id: auction_id},
        %AuctionEvent{type: :bid_placed, data: %{bid: bid = %AuctionBid{vessel_fuel_id: vessel_fuel_id}}}
      ) do
  ...
  def apply(
        state = %AuctionState{auction_id: auction_id},
        %AuctionEvent{type: :auto_bid_placed, data: %{bid: bid = %AuctionBid{vessel_fuel_id: vessel_fuel_id}}}
      ) do
    {new_product_state, _events} = process_bid(state, bid)
    new_state = AuctionState.update_product_bids(state, vessel_fuel_id, new_product_state)
...
>>>>>>> Event Sourcing 2: Non-eclectic Boogaloo
  ...
  def process_bid(state = %AuctionState{status: status, auction_id: auction_id}, bid = %AuctionBid{vessel_fuel_id: vessel_fuel_id}) do
    product_state =
