defmodule Oceanconnect.Auctions.AuctionBidProcessor do
 alias Oceanconnect.Auctions.{AuctionBidList, AuctionEvent, Command}
  ...
  defp maybe_replace_lowest_bids(current_state = %{minimum_bids: minimum_bids}, %{supplier_id: supplier_id}, %{supplier_id: supplier_id}, emit_events) do
    [lowest_minimum_bid | remaining_bids] = minimum_bids
    time_entered = DateTime.utc_now()
    updated_lowest_bids = [lowest_minimum_bid | place_remaining_min_bids(remaining_bids, time_entered, emit_events)]
    |> resolve_lowest_bids
  ...
  defp maybe_replace_lowest_bids(current_state = %{minimum_bids: minimum_bids}, _lowest_bid, _lowest_min_bid, emit_events) do
  ...
  defp place_auto_bids(minimum_bids, emit_events) do
    ...
    first_bid = case lowest_minimum_bid.min_amount == hd(remaining_bids).min_amount do
                  true -> place_auto_bid(lowest_minimum_bid, lowest_minimum_bid.min_amount, time_entered, emit_events)
      _ -> place_auto_bid(lowest_minimum_bid, hd(remaining_bids).min_amount - 0.25, time_entered, emit_events)
    end
    [first_bid | Enum.map(remaining_bids, fn(bid) -> place_auto_bid(bid, bid.min_amount, time_entered, emit_events) end)]
  end
  ...
  def process_new_bid(bid, current_state = %{lowest_bids: lowest_bids}, emit_events \\ true) do
    ...
    {lowest_bid?, updated_state} = maybe_set_lowest_bids(bid, new_state, lowest_amount, emit_events)
    {lowest_bid?, supplier_first_bid?, updated_state}
  end

  defp maybe_add_minimum_bid(current_state = %{minimum_bids: minimum_bids}, bid = %{min_amount: _min_amount}) do
  ...
  defp maybe_set_lowest_bids(bid = %{amount: nil, min_amount: min_amount}, current_state, nil, emit_events) do
  ...
  defp maybe_set_lowest_bids(%{amount: nil, supplier_id: supplier_id}, current_state = %{lowest_bids: [%{supplier_id: supplier_id} | _]}, _lowest_amount, _emit_events) do
    {false, current_state}
  end # "supplier can change minimum with no bid"
  defp maybe_set_lowest_bids(bid = %{amount: nil, min_amount: min_amount}, current_state, lowest_amount, emit_events) when min_amount > lowest_amount do
    place_auto_bid(bid, min_amount, DateTime.utc_now(), emit_events)
    {false, current_state}
  end # "auto_bid wins when only minimum provided"
  defp maybe_set_lowest_bids(bid = %{amount: nil, min_amount: amount, supplier_id: supplier_id}, current_state = %{minimum_bids: minimum_bids}, amount, emit_events) do
    ...
    maybe_resolve_minimum_bid(auto_bid, List.first(minimum_bids_without_current_supplier), current_state, amount, emit_events)
  ...
  defp maybe_set_lowest_bids(%{amount: amount, min_amount: nil}, current_state, lowest_amount, _emit_events) when amount > lowest_amount do
    {false, current_state}
  end # "new higher bid with no minimum is not added"
  defp maybe_set_lowest_bids(bid = %{amount: amount}, current_state = %{status: :pending}, lowest_amount, _emit_events) when amount < lowest_amount do
    {true, %AuctionState{current_state | lowest_bids: [bid]}}
  end # "new lowest bid replaces existing"
  defp maybe_set_lowest_bids(bid = %{supplier_id: supplier_id}, current_state = %{minimum_bids: minimum_bids}, lowest_amount, emit_events) do
    minimum_bids_without_current_supplier = Enum.reject(minimum_bids, fn(bid) -> bid.supplier_id == supplier_id end)
    maybe_resolve_minimum_bid(bid, List.first(minimum_bids_without_current_supplier), current_state, lowest_amount, emit_events)
  end
  defp maybe_set_lowest_bids(_bid, current_state, _lowest_amount, _emit_events), do: {false, current_state}

  defp maybe_resolve_minimum_bid(bid = %{min_amount: amount, time_entered: time_entered}, nil, current_state = %{lowest_bids: lowest_bids}, amount, emit_events) do
  ...
  defp maybe_resolve_minimum_bid(bid = %{amount: amount, min_amount: min_amount, time_entered: time_entered}, nil, current_state, amount, emit_events) when min_amount < amount do
  ...
  defp maybe_resolve_minimum_bid(bid = %{amount: amount, min_amount: nil}, nil, current_state = %{lowest_bids: lowest_bids}, amount, _emit_events) do
  ...
  defp maybe_resolve_minimum_bid(bid = %{amount: amount}, %{min_amount: min_amount}, current_state, min_amount, _emit_events) when amount < min_amount do
    {true, %AuctionState{current_state | lowest_bids: [bid]}}
  end # "new lower bid beats minimum"
  defp maybe_resolve_minimum_bid(bid = %{amount: amount}, %{min_amount: min_amount, time_entered: time_entered}, current_state = %{minimum_bids: minimum_bids}, _lowest_amount, emit_events) when amount < min_amount do
    place_remaining_min_bids(minimum_bids, time_entered, emit_events)
    {true, %AuctionState{current_state | lowest_bids: [bid]}}
  end # "lower bid with minimum triggers minimum bid war"
  defp maybe_resolve_minimum_bid(bid = %{amount: amount}, %{min_amount: amount}, current_state = %{lowest_bids: lowest_bids}, amount, _emit_events) do
    {false, %AuctionState{current_state | lowest_bids: lowest_bids ++ [bid]}}
  end # "minimum bid threshold is matched and min_bid supplier wins with auto_bid"
  defp maybe_resolve_minimum_bid(bid = %{amount: amount, time_entered: time_entered}, min_bid = %{min_amount: amount}, current_state, _lowest_amount, emit_events) do
  ...
  defp maybe_resolve_minimum_bid(bid = %{amount: new_amount, min_amount: new_min_amount, time_entered: time_entered}, min_bid = %{amount: old_amount, min_amount: old_min_amount}, current_state, _lowest_amount, emit_events) when not is_nil(new_min_amount) and not is_nil(old_min_amount) do
  ...
  defp maybe_resolve_minimum_bid(%{amount: new_amount, min_amount: nil, time_entered: time_entered}, min_bid = %{amount: _old_amount, min_amount: _old_min_amount}, current_state, _lowest_amount, emit_events) do
  ...
  defp place_remaining_min_bids(remaining_bids, time_entered, emit_events) do
    Enum.map(remaining_bids, fn(bid) -> place_auto_bid(bid, bid.min_amount, time_entered, emit_events) end)
  end
end
