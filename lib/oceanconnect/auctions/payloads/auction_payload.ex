defmodule Oceanconnect.Auctions.AuctionPayload do
  ...
  def get_supplier_auction_payload(auction = %Auction{}, supplier_id, state = %AuctionState{product_bids: product_bids, status: status}) do
    %AuctionPayload{
      ...
      status: status,
      solutions: SolutionsPayload.get_solutions_payload!(state, auction: auction, supplier: supplier_id),
      bid_history: get_bid_history(supplier_id, state),
      product_bids: Enum.reduce(product_bids, %{}, fn({fuel_id, product_state}, acc) ->
        Map.put(acc, fuel_id, ProductBidsPayload.get_product_bids_payload!(product_state, auction: auction, supplier: supplier_id))
      end),
      submitted_barges: BargesPayload.get_barges_payload!(state.submitted_barges, supplier: supplier_id)
    }
  end
  ...
  def get_buyer_auction_payload(auction = %Auction{}, buyer_id, state = %AuctionState{product_bids: product_bids, status: status}) do
    %AuctionPayload{
      ...
      status: status,
      solutions: SolutionsPayload.get_solutions_payload!(state, auction: auction, buyer: buyer_id),
      bid_history: [],
      product_bids: Enum.reduce(product_bids, %{}, fn({fuel_id, product_state}, acc) ->
        Map.put(acc, fuel_id, ProductBidsPayload.get_product_bids_payload!(product_state, auction: auction, buyer: buyer_id))
      end),
