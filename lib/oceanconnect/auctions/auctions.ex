defmodule Oceanconnect.Auctions do
  ...
  def solution_selectable?(
      ...
      ) when status in [:open, :decision], do: true
  ...
  def select_winning_solution(bids, product_bids, auction, comment, port_agent, user \\ nil, closed_at \\ DateTime.utc_now()) do
  ...
  def start_auction(auction = %struct{}, user \\ nil, started_at \\ DateTime.utc_now()) when is_auction(struct) do
    auction
  ...
  def expire_auction(auction = %struct{}, expired_at \\ DateTime.utc_now()) when is_auction(struct) do
    auction
  ...
  def cancel_auction(auction = %struct{}, user, canceled_at \\ DateTime.utc_now()) when is_auction(struct) do
    auction
