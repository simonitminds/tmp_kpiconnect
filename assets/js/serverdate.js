...
const ServerDate = ((serverNow => {
  ...
  const // Remember when the script was loaded.
  scriptLoadTime = Date.now();

  const // Remember the URL of this script so we can call it again during
  // synchronization.
  ...
  ["toString", "toDateString", "toTimeString", "toLocaleString",
    ...
    "toISOString", "toJSON"]
    .forEach(method => {
      ServerDate[method] = () => new Date(ServerDate.now())[method]();
    });
  ...
  function synchronize() {
    ...
    function processSample(serverNow) {
      ...
      const sample = new Offset(serverNow + precision - responseTime,
        precision);
